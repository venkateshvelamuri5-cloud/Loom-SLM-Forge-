...
var SHEETS = {
  ...
  'KB_Chunks': [
    'profile_id','chunk_index','text','src','version','embedding_json'
  ],
  'Access_Control': [
...
function doGet(e) {
  ...
  // ── Write operations via GET (bypasses CORS preflight) ──────────
  // Client sends: GAS_URL?action=register&payload=<JSON>
  // This is the primary path for all browser-originated writes.
  if (p.payload) {
    try {
      var body = JSON.parse(decodeURIComponent(p.payload));
      // Merge top-level action from URL param if not in payload
      if (!body.action && p.action) body.action = p.action;
      return _dispatch(body);
    } catch(ex) { return err('Invalid payload JSON: ' + ex.message); }
  }
...
function doPost(e) {
  // GAS POST responses cannot carry CORS headers — browser preflight fails.
  // All write operations are now also handled via doGet (payload in URL param).
  // doPost kept as fallback for server-to-server or no-cors calls.
  var body = {};
  try { body = JSON.parse(e.postData.contents); } catch(x) {}
  return _dispatch(body);
}

function _dispatch(body) {
  try {
    switch (body.action) {
      ...
      case 'deassignProfile':  return ok(handleDeassignProfile(body));
...
function handleSaveChunks(body) {
  ...
  if (chunks.length) {
    var newRows = chunks.map(function(c, idx) {
      var embJson = (c.embedding && Array.isArray(c.embedding)) ? JSON.stringify(c.embedding) : '';
      return [profileId, (batchIndex*200)+idx, c.text||'', c.src||'document', version, embJson];
    });
...
function _saveChunksToSheet(ss, profileId, chunks, version) {
  ...
  var newRows = chunks.map(function(c, idx) {
    var embJson = (c.embedding && Array.isArray(c.embedding)) ? JSON.stringify(c.embedding) : '';
    return [profileId, idx, c.text||'', c.src||'document', version, embJson];
  });
  sheet.getRange(sheet.getLastRow()+1, 1, newRows.length, 6).setValues(newRows);
}
...
//  ACCESS CONTROL
// ═══════════════════════════════════════════════════════════
// ── Deassign a specific user from a specific profile ────────────
function handleDeassignProfile(body) {
  var ss       = SpreadsheetApp.openById(SPREADSHEET_ID);
  var sheet    = ss.getSheetByName('Access_Control');
  if (!sheet) return { success:true, removed:0 };

  var email     = (body.userEmail||'').toLowerCase().trim();
  var profileId = body.profileId || '';
  if (!email || !profileId) return { success:false, error:'userEmail and profileId required' };

  var rows    = sheet.getDataRange().getValues();
  var removed = 0;
  for (var i = rows.length - 1; i >= 1; i--) {
    if ((rows[i][0]||'').toLowerCase().trim() === email && rows[i][1] === profileId) {
      sheet.deleteRow(i + 1);
      removed++;
    }
  }
  return { success:true, removed:removed, email:email, profileId:profileId };
}
